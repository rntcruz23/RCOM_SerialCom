...
#define I1 0x40
#define MAXPACK 256
#define MAX MAXPACK*3
#define DELAYS 1.25
#define DELAY DELAYS*1000000
...
int info_construct(char *buf,char *packet, int packet_size,int n);	//Insert packet(buf) in trama
...
time_t getTime(char type,int length,char *camada_app);
int createApplevelEnd(char *appLevel);
...
unsigned int sequenceNumber; 		/*Número de sequência da trama: 0, 1 : trama que recetor espera receber*/
...
int main(int argc, char** argv){
  ...
  */
    ...
    if (app.fd <0) {perror(argv[1]); exit(-1); }
  ...
  */
	...
	if (llopen(app.fd,app.status) >= 0){//emissor -> send SET    recetor -> wait SET send ACK 	return: -1 on failure	
	...
	else printf("Problema na inicializacao (llopen())\n");
	llclose(app.fd);
  ...
  */ 
	...
	printf("It took: %lfs to transfer file\n",time_spent);
    return 0;
}
int llopen (int porta, int status){
	char trama[5];	
...
int wait_ack(int porta){				//return: 0 -> max trys | 1 -> ack
	...
	while (!flag_ack) {       			/* loop for input */
		if (!flag_alarm){
			...
			write(porta,trama, i);
			send_count++;
			if (send_count > linkL.numTransmissions) break;
			if(send_count > 1)
				printf("Try %d\n",send_count);
			flag_alarm = 1;
			alarm(linkL.timeout);	
...
int test_bcc1(char *trama){ 			//Return 1 -> trama de controlo || Return 0 -> trama de informaçao	
...
char proc_C(char *trama, int porta){											//Return flag C
	...
	switch (c){		
		...
		case C_DISC:			//Close connection
			...
			break;
		case C_UA: flag_ack = 1; printf("UA Received\n");								//Wait for data
			break;
		case C_REJ:
			break;
		default : printf("Frame Unknown!");
	}
	return c;
}
int info_construct(char *buf,char *packet, int packet_size,int n){		//Insert packet(buf) in trama
...
int leitura_trama (char *trama,int porta){
	...
	while (!(fim) && (cnt < MAX) && !(!flag_alarm*(app.status == A_SENDER))){
		read(porta,&ch,1); 
		if((cnt == 1) && (ch == FR_FLAG)) cnt = 0;					//FR_FLAG seguido de FR_FLAG
...
int sendFile(char *filename,int porta){
  ...
  new_times.modtime = foo.st_mtime;   /* set mtime to current time */
	...
	int applevelSize = createApplevelStart(filename,filesize,beforeStuff);	//Cria camada de aplicaçao F|A|C|B[DADOS]B|F
	applevelSize = stuffFrame(beforeStuff,applevelSize,appLevel);			//Stuff frame and create BCC2 F|A|C|B[DADOS|B]F
	...
	if (f) {
		while ((nread = fread(chunck, 1, sizeof chunck, f)) > 0)
		{
			...
			npack += nread;
			clear();
			printf("\tPacket: #%d\tSent: %0.0f%%\n[",n,((float)npack/(float)filesize)*100);
			while (j++ < 50){
	...
	applevelSize = createApplevelEnd(beforeStuff);	//Cria camada de aplicaçao F|A|C|B[DADOS]B|F
	applevelSize = stuffFrame(beforeStuff,applevelSize,appLevel);			//Stuff frame and create BCC2 F|A|C|B[DADOS|B]F
	if(!wait_sequence(porta,linkL.sequenceNumber,appLevel,applevelSize))
		return -1;	//wait rr1
	...
	//CRIAR TRAMAS DE INFORMAÇAO -> camada de aplicaçao [DADOS] -> [C|N|L2|L1|P1...Pk]
	//llwrite -> acrescentar cabeçalhos e enviar -> [F|A|C|B]DADOS[B|F] -> n++;
...
int wait_sequence(int porta,int Ns,char *appLevel,int applevelSize){
	...
	while (!flag_ack) {       			/* loop for input */
		if (!flag_alarm){
			llwrite(porta,appLevel,applevelSize);			//Adiciona cabeçalhos e envia [F|A|C|B]DADOS[B|F]
			flag_alarm = 1;
			send_count++;
			if (send_count > linkL.numTransmissions) break;
			if(send_count > 1)
...
int createApplevelEnd(char *appLevel){
	int i,j;
	time_t at= foo.st_atime;
	time_t mt= foo.st_mtime;
	appLevel[0] = C_END;
	appLevel[1] = 0x02;
	appLevel[2] = (char) sizeof(at);

	for(i = sizeof(at) + 2; i > 2; i--){
		appLevel[i] = (char)at;
		at = at >> 8;	
	...
	appLevel[++i]= (char) sizeof(mt);
	j=i;
	for(i = sizeof(mt) + j; i > j; i--){
		appLevel[i] = (char)mt;
		mt = mt >> 8;
	}
	return i + sizeof(mt);
...
int llread(int fd, char * buffer){
	int readsize,packet_size,at_length,mt_length;
	time_t at,mt;
	char rr_trama[5];
	char *packet;
	...
	while (c_control != C_DISC){
		if ((readsize = leitura_trama(buffer,fd)) != -1){
			...
			if (test_bcc1(buffer) != -1){	//BCC1 correto
				c_control = buffer[2];
				if((c_control & 1) == 1) continue;
				if ((int)c_control>>6 == linkL.sequenceNumber){	//Trama recebida tem numero de sequencia esperado
					...
					totalReceived += packet_size;
					  ...
					  switch(c_info){
						case C_START: 						
							...
							if ((filesize = getFileLength(type,lengthSize,packet + 3)) == -1)
								return -1;	
							type = packet[3 + lengthSize];
							lengthName = packet[3 + lengthSize + 1];
							if(getFileName(type,lengthName,packet + 3 + lengthSize + 2 ,filename) == -1)
								return -1;
							printf("Filesize: %d\tFilename: %s\n",filesize,filename);
							output_file = open(filename,O_RDWR|O_CREAT);			//ficheiro de saida criado
							break;
						case C_I: 
							nFrame = packet[1];
							frameSize = packet[2]*256 + packet[3];
							if(frameSize < MAXPACK && frameSize > 256)
							  frameSize = frameSize + 256;
							
							received += frameSize;
							clear();
							printf("framesize: %d frame: %d\n",frameSize,nFrame);
							printf("\tReceived: %0.0f%%\n[",((float)received/(float)filesize)*100);
							while (j++ < 50){
						...
						case C_END:
							...
							type = packet[1];
							at_length = packet[2];
							at = getTime(type,at_length,packet + 3);
							type = packet[3 + at_length];
							mt_length = packet[3 + at_length + 1];
							mt = getTime(type,mt_length,packet + 3 + at_length + 2);
							new_times.actime = at;
							new_times.modtime = mt;
							utime(filename,&new_times);
							break;
						default: return -1;	
						}			
						linkL.sequenceNumber ^= 1;		       //Mudar numero de trama esperado
						rr_construct(rr_trama,linkL.sequenceNumber);   //Pedir trama seguinte
					}
					else rej_construct(rr_trama,linkL.sequenceNumber); //BCC2 errado
				}else rr_construct(rr_trama,linkL.sequenceNumber);  //Numero de sequencia nao esperado, pedir novamente
			}else rej_construct(rr_trama,linkL.sequenceNumber);         //BCC1 errado
			usleep(DELAY);
...
int llclose(int fd){
	...
	else if (app.status == A_RECEIVER){			// DISC recebido -> send DISC -> wait_ack();
		write(fd,rr_trama,5);
		if (!wait_ack(fd))
			return -1;
	}
